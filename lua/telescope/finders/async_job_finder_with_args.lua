...
return function(opts)
  ...
  local fn_command = function(fn_opts)
    local command_list = opts.command_generator(fn_opts)
  ...
  local callable = function(_, fn_opts, process_result, process_complete)
    if job then
      job:close(true)
    end

    local job_opts = fn_command(fn_opts)
